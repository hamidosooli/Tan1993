...
def rl_agent(beta=0.8):
    Q1 = np.zeros((Row_num, Col_num, Row_num, Col_num, nA))
    Q2 = np.zeros((Row_num, Col_num, Row_num, Col_num, nA))
    ...
    for eps in range(NUM_EPISODES):
        ...
        prey1_pose = [np.random.choice(range(Row_num)), np.random.choice(range(Col_num))]
        prey2_pose = [np.random.choice(range(Row_num)), np.random.choice(range(Col_num))]
        ...
        while True:
            ...
            T_prey1.append(prey1_pose)
            T_prey2.append(prey2_pose)

            hunter1_sensation = [np.subtract(prey_pose, hunter1_pose), np.subtract(hunter2_pose, hunter1_pose)]
            hunter2_sensation = [np.subtract(prey_pose, hunter2_pose), np.subtract(hunter1_pose, hunter2_pose)]

            hunter1_probs = Boltzmann(Q1[hunter1_sensation[0][0], hunter1_sensation[0][1],
                                         hunter1_sensation[1][0], hunter1_sensation[1][1], :])
            hunter1_action = np.random.choice(ACTIONS, p=hunter1_probs)
            hunter2_probs = Boltzmann(Q2[hunter2_sensation[0][0], hunter2_sensation[0][1],
                                         hunter2_sensation[1][0], hunter2_sensation[1][1], :])
            hunter2_action = np.random.choice(ACTIONS, p=hunter2_probs)
            prey_action = np.random.choice(ACTIONS)

            hunter1_sensation_prime = [transition(hunter1_sensation[0], hunter1_action),
                                       transition(hunter1_sensation[1], hunter2_action)]
            hunter2_sensation_prime = [transition(hunter2_sensation[0], hunter2_action),
                                       transition(hunter2_sensation[1], hunter1_action)]
            ...
            Q1[int(hunter1_sensation[0][0]),
               int(hunter1_sensation[0][1]),
               int(hunter1_sensation[1][0]),
               int(hunter1_sensation[1][1]), hunter1_action] += beta * (re1 +
                                          gamma * np.max(Q1[int(hunter1_sensation_prime[0][0]),
                                                            int(hunter1_sensation_prime[0][1]),
                                                            int(hunter1_sensation_prime[1][0]),
                                                            int(hunter1_sensation_prime[1][1]), :]) -
                                          Q1[int(hunter1_sensation[0][0]),
                                             int(hunter1_sensation[0][1]),
                                             int(hunter1_sensation[1][0]),
                                             int(hunter1_sensation[1][1]), hunter1_action])

            Q2[int(hunter2_sensation[0][0]),
               int(hunter2_sensation[0][1]),
               int(hunter2_sensation[1][0]),
               int(hunter2_sensation[1][1]), hunter2_action] += beta * (re1 +
                                          gamma * np.max(Q2[int(hunter2_sensation_prime[0][0]),
                                                            int(hunter2_sensation_prime[0][1]),
                                                            int(hunter2_sensation_prime[1][0]),
                                                            int(hunter2_sensation_prime[1][1]), :]) -
                                                 Q2[int(hunter2_sensation[0][0]),
                                                    int(hunter2_sensation[0][1]),
                                                    int(hunter2_sensation[1][0]),
                                                    int(hunter2_sensation[1][1]), hunter2_action])
            ...
            if ((hunter1_sensation_prime[0] == [0, 0] and hunter2_sensation_prime[0] == [0, 0]) or
                (np.linalg.norm(hunter1_sensation_prime[0]) == 1 and np.linalg.norm(hunter2_sensation_prime[0]) == 1)):
