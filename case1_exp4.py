...
def reward_hunter(hunter_sensation_prime):
...
def reward_scout(scout_sensation_prime):
    if abs(scout_sensation_prime[0]) <= Scout_VFD and abs(scout_sensation_prime[1]) <= Scout_VFD:
        re = 1
    else:
        re = -.1
    return re
...
def rl_agent(beta=0.8):
    ...
    Q_scout = np.zeros((2*Row_num+1, 2*Col_num+1, nA))
    ...
    for eps in range(NUM_EPISODES):
        ...
        while True:
            ...
            scout_probs = Boltzmann(Q_scout[Row_num+scout_sensation[0], Col_num+scout_sensation[1], :])
            ...
            re_hunter = reward_hunter(hunter_sensation_prime)
            re_scout = reward_scout(scout_sensation_prime)

            A_hunter.append(hunter_action)
            A_scout.append(scout_action)
            A_prey.append(prey_action)
            R_hunter.append(re_hunter)
            R_scout.append(re_scout)
            if default_sensation != [row_lim, column_lim]:
                R_prime.append(re_hunter)
                counter += 1
            if counter == 1:
                s = t_step
            Q[Row_num+hunter_sensation[0],
              Col_num+hunter_sensation[1], hunter_action] += beta * (re_hunter +
                                                 gamma * np.max(Q[Row_num+hunter_sensation_prime[0],
                                                                  Col_num+hunter_sensation_prime[1], :]) -
                                                 Q[Row_num+hunter_sensation[0],
                                                   Col_num+hunter_sensation[1], hunter_action])

            Q_scout[Row_num+scout_sensation[0],
                    Col_num+scout_sensation[1], scout_action] += beta * (re_scout +
                                                 gamma * np.max(Q_scout[Row_num+scout_sensation_prime[0],
                                                                        Col_num+scout_sensation_prime[1], :]) -
                                                 Q_scout[Row_num+scout_sensation[0],
                                                         Col_num+scout_sensation[1], scout_action])
            ...
            if hunter_sensation_prime == [0, 0]:
                ...
                steps.append(t_step)
                see_steps.append(s)
                rewards_hunter.append(sum(R_hunter))
    ...
    return T_hunter, T_scout, T_prey, A_hunter, A_scout, A_prey, rewards_hunter, rewards_scout, steps, see_rewards, see_steps, Q
...
T_hunter, T_scout, T_prey, A_hunter, A_scout, A_prey, rewards_hunter, rewards_scout, steps, see_rewards, see_steps, Q = rl_agent(beta=0.8)
    ...
    # rewards_hunter_runs.append(rewards_hunter)
...
# rewards_hunter_runs = np.asarray(rewards_hunter_runs)
# rewards_scout_runs = np.asarray(rewards_scout_runs)
...
with h5py.File(f'Tan1993_case1_runs_exp4_sim.hdf5', "w") as f:
    ...
    f.create_dataset('rewards_hunter', data=rewards_hunter_runs)
