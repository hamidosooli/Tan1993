...
with h5py.File('multi_agent_Q_learning.hdf5', 'r') as f:
    ...
    for idx in range(f['victims_num']):
        victims_Traj.append(f[f'victim{idx}_trajectory'])

    if run_animate:
        animate(rescue_team_Traj, victims_Traj, f['RS_VFD'], wait_time=0.5)

    rescue_team_legends = []

    plt.figure('reward')
    for idx in range(len(f['RS_VFD'])):
        plt.plot(rescue_team_RewSum[idx])
    ...
    for idx in range(len(f['RS_VFD'])):
        plt.plot(rescue_team_RewSum_seen[idx])
    plt.xlabel('Number of episodes')
    ...
    plt.figure('steps')
    for idx in range(len(f['RS_VFD'])):
        plt.plot(rescue_team_Steps[idx])
    plt.xlabel('Number of episodes')
    ...
    for idx in range(len(f['RS_VFD'])):
        plt.plot(rescue_team_Steps_seen[idx])
    plt.xlabel('Number of episodes')
