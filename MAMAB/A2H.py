...
if __name__ == '__main__':
    ...
    for run in range(numRuns):
        mab = TR[run,:]
        n_out, s_out, I_out, imm_regret, p_out = play(Adj, T, mab)
        regret[:, :, run] = imm_regret
        avg_regret = np.mean(regret, axis=2)
        print(run)

    plt.figure('house A3 and A2 10k arm=10 kappa= 0.02 gamma=1.9 may 19 6 ents agent 2 and 3connect')
    data = h5py.File('A32H10k.hdf5', "w")
